...
public class OrderMgr {
	...
	public void insertOrder(OrderBean oderbean){
		try {
			conn = ds.getConnection();
			String sql = "insert into shop_order(product_no,quantity,sdate,state,id) values(?,?,now(),?,?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, oderbean.getProduct_no());
			pstmt.setString(2, oderbean.getQuantity());
			pstmt.setString(3, "1");
			pstmt.setString(4, oderbean.getId());
			pstmt.executeUpdate();
		} catch (Exception e) {
